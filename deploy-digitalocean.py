...
@click.option(
    '--size',
    help='Size of droplet to deploy (default: 2gb)',
    default='2gb'
)
@click.option(
    '--cloud-config',
...
def main(api_token, region, size, cloud_config):
    ''' Attempts to deploy a Ogawa instance to Digital Ocean. '''
    logging.basicConfig(
        level=logging.INFO,
    ...
    # Print IP and exit.
    droplet.load()
    log.info('New Ogawa host should be running at %s', droplet.ip_address)
